...
public class AttendanceRecord implements Serializable
{
	...
	public String toString()
	{
		...
		if(presentees.size()==attendance.size())		//ALL are present
			toReturnStr.append("Presentees[Count="+presentees.size()+"]: ").append("ALL(1 to "+presentees.size()+")").append(System.getProperty("line.separator"));
		else
			toReturnStr.append("Presentees[Count="+presentees.size()+"]: ").append(presentees.size()==0?"None":presentees.toString()).append(System.getProperty("line.separator"));
		
		if(absentees.size()==attendance.size())		//ALL are absent
			toReturnStr.append("Absentees[Count="+absentees.size()+"]: ").append("ALL(1 to "+absentees.size()+")").append(System.getProperty("line.separator"));
		else
			toReturnStr.append("Absentees[Count="+absentees.size()+"]: ").append(absentees.size()==0?"None":absentees.toString()).append(System.getProperty("line.separator"));
		
		toReturnStr.append("Medical Leaves[Count="+medicalLeaves.size()+"]: ").append(medicalLeaves.size()==0?"None":medicalLeaves.toString()).append(System.getProperty("line.separator"));
		toReturnStr.append("Other Reasons Leaves[Count="+otherReasonsLeave.size()+"]: ").append(otherReasonsLeave.size()==0?"None":otherReasonsLeave.toString()).append(System.getProperty("line.separator"));
